import { parseRange, toRef, displayValue, applyNumFmt } from '@ai-studio/formula';

export const CHART_TYPES = ['column', 'bar', 'line', 'area', 'pie', 'donut'];
 ...
 *
 * Both columns are selected — the dark row is the same eight hues re-stepped for a
 * dark surface, not an automatic inversion. Validated with the data-viz palette
 * checker against both surfaces: worst adjacent CVD ΔE 9.1 light / 8.4 dark,
 * worst adjacent normal-vision ΔE 19.6 / 19.3.
 *
 * Three light slots sit under 3:1 contrast on white, so the renderer owes the
 * relief the checker demands: a legend for every multi-series chart, direct value
 * labels, and the underlying numbers as a table in `AI.md`.
 */
const PALETTE = {
...
/** Fill in defaults and clamp anything that would break the renderer. */
export function normalizeChartSpec(spec) {
  const raw = spec && typeof spec === 'object' ? spec : {};
  const type = CHART_TYPES.includes(raw.type) ? raw.type : 'column';
  ...
  let series = Array.isArray(raw.series)
    ? raw.series
        ...
        .map((s) => ({ name: String(s.name ?? ''), values: s.values.map(toNumberOrNull) }))
    : [];
  ...
  if (series.length > MAX_SERIES) {
    ...
    const length = Math.max(...rest.map((s) => s.values.length), 0);
    const summed = Array.from({ length }, (_, i) =>
      rest.reduce((total, s) => total + (Number(s.values[i]) || 0), 0)
    );
    series = [...kept, { name: '기타', values: summed }];
  }
  ...
  return {
    ...
    options: {
      stacked: !!raw.options?.stacked,
      legend: raw.options?.legend !== false,
      grid: raw.options?.grid !== false,
      // Selective by default: a number on every point is noise.
      valueLabels: ['none', 'max', 'ends', 'all'].includes(raw.options?.valueLabels)
        ? raw.options.valueLabels
        : 'max',
      yTitle: raw.options?.yTitle ? String(raw.options.yTitle) : '',
      numberFormat: raw.options?.numberFormat ? String(raw.options.numberFormat) : '',
    },
  };
}
...
export function parseChartBlock(md) {
  const m = String(md ?? '').match(/```chart[^\n]*\n([\s\S]*?)```/);
  if (!m) return null;
  try {
    return normalizeChartSpec(JSON.parse(m[1]));
  } catch {
    return null;
  }
}
...
export function serializeChartBlock(spec) {
  return `\`\`\`chart\n${JSON.stringify(normalizeChartSpec(spec), null, 2)}\n\`\`\``;
}

/**
 * Pull chart data out of a sheet range.
 *
 * The first row (or column) is treated as the label axis and the first cell of
 * each series as its name, which is what a user selecting a table expects.
 */
export function chartDataFromRange(sheet, rangeSpec, { orientation = 'columns' } = {}) {
  const r = parseRange(rangeSpec);
  if (!r) return { labels: [], series: [] };

  const at = (col, row) => sheet.cells?.[toRef(col, row)];
  const text = (col, row) => displayValue(at(col, row));
  const num = (col, row) => {
    const cell = at(col, row);
    return cell && typeof cell.v === 'number' && cell.t !== 'e' ? cell.v : null;
  };

  if (orientation === 'columns') {
    // Columns are series: header row names them, first column labels the points.
    const labels = [];
    for (let row = r.start.row + 1; row <= r.end.row; row++) labels.push(text(r.start.col, row));
    const series = [];
    for (let col = r.start.col + 1; col <= r.end.col; col++) {
      const values = [];
      for (let row = r.start.row + 1; row <= r.end.row; row++) values.push(num(col, row));
      series.push({ name: text(col, r.start.row) || toRef(col, r.start.row), values });
    }
    return { labels, series };
  }

  const labels = [];
  for (let col = r.start.col + 1; col <= r.end.col; col++) labels.push(text(col, r.start.row));
  const series = [];
  for (let row = r.start.row + 1; row <= r.end.row; row++) {
    const values = [];
    for (let col = r.start.col + 1; col <= r.end.col; col++) values.push(num(col, row));
    series.push({ name: text(r.start.col, row) || toRef(r.start.col, row), values });
  }
  return { labels, series };
}
 ...
 *
 * If the range yields nothing usable — a bad address, a single column with no
 * series, a sheet that no longer has those cells — any inline data on the spec is
 * kept instead of rendering an empty chart. Same principle as the rest of the
 * format: a stale or hand-edited file still shows what it can.
 */
export function resolveChartSpec(spec, sheet) {
  const normalized = normalizeChartSpec(spec);
  if (!normalized.range || !sheet) return normalized;

  const data = chartDataFromRange(sheet, normalized.range, { orientation: normalized.orientation });
  const usable = data.series.some((s) => s.values.some((v) => v !== null));
  if (!usable) return normalized;

  return normalizeChartSpec({ ...normalized, labels: data.labels, series: data.series });
}
 ...
 *
 * SVG rather than canvas so the same output works in the browser, in the file
 * inspector, and (as markup) in an export. Marks follow the data-viz specs: bars
 * capped at 24px with a 4px rounded data-end, 2px lines, hairline solid
 * gridlines one step off the surface, a 2px surface gap between touching fills,
 * and value labels only where they earn their place.
 */
 ...
 *
 * This is the accessibility relief the palette check requires — three light-mode
 * slots sit below 3:1 contrast — and it is also the reason a chart in this format
 * is legible to a model at all: `AI.md` carries the figures, not a picture.
 */
export function chartToMarkdownTable(spec) {
  const chart = normalizeChartSpec(spec);
  if (!chart.series.length) return '_차트 데이터가 없습니다_';

  const header = ['구분', ...chart.series.map((s, i) => s.name || `계열 ${i + 1}`)];
  const lines = [`| ${header.join(' | ')} |`, `|${header.map(() => '---').join('|')}|`];

  const rows = chart.labels.length || Math.max(...chart.series.map((s) => s.values.length));
  for (let i = 0; i < rows; i++) {
    const cells = [chart.labels[i] || `${i + 1}`];
...
export function describeChart(spec) {
  const chart = normalizeChartSpec(spec);
  const type = CHART_TYPE_LABELS[chart.type] ?? chart.type;
  const stacked = chart.options.stacked ? ' 누적' : '';
  const seriesNames = chart.series.map((s, i) => s.name || `계열 ${i + 1}`).join(', ');
  const points = chart.labels.length || Math.max(...chart.series.map((s) => s.values.length), 0);
  const source = chart.range ? ` (범위 \`${chart.range}\`)` : '';
  return `${type}${stacked} 차트${chart.title ? ` "${chart.title}"` : ''} — 계열 ${chart.series.length}개(${seriesNames}), 항목 ${points}개${source}`;
}
